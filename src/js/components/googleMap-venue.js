...
function initializeVenueMap() {
	...
	const htmlMapElement = document.getElementById("map__canvas");
	...
	for (let i = 0; i < hotels.length; i += 1) {
		...
		infoWindowContent.push(`<div class="info_content">
				...
				<h3>${hotels[i].name}</h3>
				${hotels[i].address.street} </br>${hotels[i].address.city}, ${
			hotels[i].address.state
		} ${hotels[i].address.zip}
				</address>
			</div>`);
		...
		buttons[i].addEventListener(
			"click",
			generateTriggerCallback(marker, "click")
		);
